...
json_escaped_char(epc_parser_list * list)
{
    ...
    epc_parser_t * simple_escape = epc_and_l(
        list,
        "simple_escape",
        2,
    ...
    epc_parser_t * unicode_escape = epc_and_l(
        list,
        "unicode_escape",
        2,
    ...
    return epc_or_l(list, "escaped_char", 2, simple_escape, unicode_escape);
}

static epc_parser_t *
json_quoted_string(epc_parser_list * list)
{
    epc_parser_t * string_content_char = epc_or_l(
        list, "string_content_char", 2, epc_none_of_l(list, "non_quote_non_backslash", "\"\\"), json_escaped_char(list)
    );

    epc_parser_t * inner_string = epc_many_l(list, "inner_string", string_content_char);
...
{
    return epc_or_l(list, "boolean", 2, epc_string_l(list, "true", "true"), epc_string_l(list, "false", "false"));
}
...
{
    return epc_parser_fwd_decl_l(list, "json_value_ref");
}

static epc_parser_t *
json_array_parser_ref(epc_parser_list * list)
{
    return epc_parser_fwd_decl_l(list, "json_array_ref");
}

static epc_parser_t *
json_object_parser_ref(epc_parser_list * list)
{
    return epc_parser_fwd_decl_l(list, "json_object_ref");
}
