...
rpc_send_response(rpc_server_st * svr, struct json_object * id, struct json_object * result)
{
    if (id)
    {
        char const * id_str = json_object_to_json_string_ext(id, JSON_C_TO_STRING_PLAIN);
        fprintf(stderr, "[LSP] sending response for id=%s\n", id_str);
    }
...
rpc_dispatch(rpc_server_st * svr, struct json_object * msg)
{
    ...
    {
        char const * id_str = json_object_to_json_string_ext(id, JSON_C_TO_STRING_PLAIN);
        fprintf(stderr, "[LSP] dispatching '%s' (id=%s)\n", method_name, id_str);
    }
    else
    {
        fprintf(stderr, "[LSP] dispatching '%s' (notification)\n", method_name);
    }
