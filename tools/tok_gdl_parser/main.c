...
main(int argc, char ** argv)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        {
            printf("tok_gdl_parser version: %s\n", TOK_GDL_PARSER_VERSION);
    ...
    epc_ast_result_t token_build = epc_ast_build(
        session.result.data.success, tokenizer_registry, &tokenizer_ctx
    );
