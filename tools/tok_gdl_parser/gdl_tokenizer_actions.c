...
handle_simple_token(epc_ast_builder_ctx_t * ctx, epc_cpt_node_t * node, void ** children, int count, void * user_data)
{
    ...
    if (s && view.len > 0)
    {
        switch (s[0])
        {
        case '=': id = TOKEN_EQUALS;    break;
        case ';': id = TOKEN_SEMICOLON;  break;
        case '|': id = TOKEN_PIPE;       break;
        case '@': id = TOKEN_AT;         break;
        case '(': id = TOKEN_LPAREN;     break;
        case ')': id = TOKEN_RPAREN;     break;
        case ',': id = TOKEN_COMMA;      break;
        case '-': id = TOKEN_MINUS;      break;
        case '*': id = TOKEN_STAR;       break;
        case '+': id = TOKEN_PLUS;       break;
        case '?': id = TOKEN_QUESTION;   break;
        }
    }
...
)
{
    ...
    gdl_tokenizer_ctx_t * tctx = user_data;
    epc_parser_input_view_t view = epc_cpt_node_get_input_view(node);
    epc_token_list_add(tctx->tokens, TOKEN_CHAR_RANGE, view);
    tctx->last_match = node;
}
...
gdl_tokenizer_hook_registry_init(epc_ast_hook_registry_t * registry, gdl_tokenizer_ctx_t * ctx)
{
    ...
    epc_ast_hook_registry_set_action(registry, TOKENIZER_ACTION_CHAR_RANGE, handle_char_range);
